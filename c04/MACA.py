...
for ind in tqdm(range(sim_fut.pr.shape[0])): # loop over all days in sim_fut
...
sim_fut_fine.to_netcdf(var+model_name+'_'+ssps+'_90arcsec_'+str(year_fut_start)+"-"+str(year_fut_end)+years_date+'.nc')

#
## load the existing netcdf file as a Dataset object
#ds = xr.open_dataset('sim_fut_fine.nc')
#
...
#ds.to_netcdf('sim_fut_fine_updated.nc')
#
## now bias adjust the results 
#ds = xr.open_dataset("sim_fut_fine_updated.nc")
## fit the model 
#model = PointWiseDownscaler(BcsdPrecipitation(return_anoms=False))
#sim_hist_fine = sim_hist.regrid.regrid(ds_fine_resoluation, method="linear")
#obs_fine_regrid = obs_fine.regrid.regrid(ds_fine_resoluation, method="linear")
##sim_hist_fine.pr
#model.fit(sim_hist_fine.pr, obs_fine_regrid.pr)
## do the fitting :
#dd= model.predict(ds.pr).load()
#
#empty_da = xr.DataArray(dd)
## optionally, you can also specify the dimensions and coordinates for the DataArray
#dims = ('time', 'lon', 'lat')
#coords = {'time': ds.time.data , 'lon': ds.lon.data,
#           'lat': ds.lat.data}
#
#sim_fut_fine = xr.DataArray(empty_da, name='pr', dims=dims, coords=coords)
## write the DataArray to a netcdf file 
#sim_fut_fine.to_netcdf('sim_fut_fine_biasadjusted.nc')
#
## load the existing netcdf file as a Dataset object
#ds = xr.open_dataset('sim_fut_fine_biasadjusted.nc')
#
## set or update the attributes for the time dimension
#ds.time.attrs['standard_name'] = 'time'
#ds.time.attrs['long_name'] = 'time'
...
## write the Dataset to a new netcdf file with the updated attributes
#years_date= '_'+str(lon0)+'_'+str(lon1)+'_'+str(lat0)+'_'+str(lat1)
#ds.to_netcdf(var+model_name+'_'+ssps+'_90arcsec_'+str(year_fut_start)+"-"+str(year_fut_end)+years_date+'.nc')
#print("FINISHED-----------------------------------------")
#empty_array.flush()
#del empty_array
#